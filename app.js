...
import { getDatabase, ref, set, get, onValue, update, push, runTransaction } from "https://www.gstatic.com/firebasejs/12.18.0/firebase-database.js";
...
// Kelime hakkını sadece bu fonksiyon tüketir. Emojiler buraya hiç girmez.
async function submitCurrentWord(rawWord){if(gameFinished||!roomId||!playerNumber)return;const word=String(rawWord||"").trim();const normalized=normalizeWord(word);if(!normalized){alert("Geçerli bir kelime yaz.");return;}const roomRef=ref(db,`rooms/${roomId}`);const snap=await get(roomRef);if(!snap.exists())return;const r=snap.val();if(r.finished||r.round!==currentRound)return;if(mySubmitted(r)){setStatus("Bu adımda kelimeni zaten gönderdin.");return;}
  const usedRef=ref(db,`rooms/${roomId}/usedWords/${normalized}`);
  const tx=await runTransaction(usedRef,current=>current===null?{round:currentRound,word}:current);
  if(!tx.committed||!tx.snapshot.exists()||tx.snapshot.val()?.round!==currentRound||tx.snapshot.val()?.word!==word){setStatus("🚫 Bu kelime daha önce kullanıldı. Başka bir kelime seç.");playTone("error");return;}
  await set(ref(db,`rooms/${roomId}/${playerPath()}/words/${currentRound}`),word);
  wordInput.value="";setStatus("✅ Kelimen gönderildi. Rakibin durumu tabloda görünüyor.");playTone("success");await checkRound();
}
...
function renderTable(r){const p1=r.player1||{},p2=r.player2||{},a=p1.words?.[currentRound],b=p2.words?.[currentRound];tableName1.textContent=p1.name||"Oyuncu 1";tableName2.textContent=p2.name||"Oyuncu 2";currentName1.textContent=p1.name||"Oyuncu 1";currentName2.textContent=p2.name||"Oyuncu 2";tableWord1.textContent=a||"—";tableWord2.textContent=b||"—";tableStatus1.textContent=a?"✅ HAZIR":"🤔 Düşünüyor...";tableStatus2.textContent=b?"✅ HAZIR":"🤔 Düşünüyor...";word1.textContent=a||"—";word2.textContent=b||"—";currentWords.classList.toggle("hidden",!(a||b));}
...
function finishGame(r){if(gameFinished)return;gameFinished=true;stopTimer();game.classList.add("hidden");waiting.classList.add("hidden");result.classList.remove("hidden");const[s1,s2]=calculateScores(r),round=r.winnerRound||0; if(round){const a=r.player1.words?.[round],b=r.player2.words?.[round];resultBadge.textContent="🎉";resultTitle.textContent="KELİME BULUNDU — KAZANILDI!";resultText.textContent=`${round}. adımda eşleşme bulundu. Oyun sonlandırıldı.`;finalScores.innerHTML=`<div><b>${escapeHtml(r.player1.name)}</b>: ${escapeHtml(a)}</div><div><b>${escapeHtml(r.player2.name)}</b>: ${escapeHtml(b)}</div><hr><div>🏆 ${s1} - ${s2}</div>`;playTone("success");}else{resultBadge.textContent="😔";resultTitle.textContent="KELİME BULUNAMADI";resultText.textContent="5 adım tamamlandı, eşleşen kelime bulunamadı.";finalScores.innerHTML=`<div>${escapeHtml(r.player1.name)}: ${s1} puan</div><div>${escapeHtml(r.player2.name)}: ${s2} puan</div>`;playTone("error");}}
